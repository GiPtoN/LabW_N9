...
namespace Number13
{
    ...
    {
        ///Определить долю в процентах слов, начинающихся на разные буквы.
    ...
    public static class SupportMethods
    {
        ...
        {
            string path = "FileN13.txt";
            File.Delete(path);
            var Phile = new StreamWriter(path, true);
            for (int i = 0; i < answers.Count; i++)
            {
                string str = $"Answer: {answers[i].Anss} - {answers[i].Count} \t Result: {(answers[i].Count * 100) / AnsCount}%";
                Phile.WriteLine(str);
...
namespace Number15
{
    public static class Program
    {
        ...
        public static void Start()
        {
            ...
            SupportMethods.PrintParticipant(Sum);
        }
    }

    public static class SupportMethods
    {
        public static void PrintParticipant(int Ans)
        {
            string path = "FileN15.txt";
            File.Delete(path);
            var Phile = new StreamWriter(path, true);
            string str = $"{Ans}";
            Phile.WriteLine(str);
